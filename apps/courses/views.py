...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def question_import_json_package(request, course_id):
    """Import a JSON package and link its resolved questions to this course."""
    course = _get_course_or_404(course_id)
    _check_course_owner(course, request.user)
    ...
    return import_json_package_for_course(request, course=course, tree_node=tree_node)
