...
- 每次模型请求超时 300 秒。
- 单模式流程最多包含 Qwen、DeepSeek 第一阶段和 DeepSeek 第二阶段；每阶段允许一次重试，因此单次 300 秒请求的理论总预算可达 1800 秒。按用户确认的双倍安全余量，Celery 软超时设为 3800 秒，硬超时设为 3900 秒，确保任务层不会先于完整仲裁流程被终止。
...
为 `question_id + mode` 建立 4200 秒 Redis 任务锁。同一题目同一模式正在运行时返回现有任务状态，不重复调用模型；不同模式仍可分别手工触发。任务无论成功、失败或异常退出，都在 `finally` 中主动释放锁，TTL 只作为进程崩溃时的兜底。“一键全部 AI 处理”只负责编排用户明确触发的独立操作。
