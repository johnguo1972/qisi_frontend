...
- Qwen 结果先经过确定性的答案标准化、题目完整性预检和模式内容校验。标准化覆盖选择题标签、判断值、数值和单位等可确定情况，不使用模糊子串猜测；内容门检查必填字段、选项存在性、最终答案一致性、缺失条件和题面外事实。
- 快速路径只适用于题干非空、完整的客观题。已识别的选择题必须至少有两个非空选项，标签必须是互异的单字母且内容互异；主观/自由作答题、题面明确缺少条件、显式低于阈值或格式非法的置信度都必须进入独立验证。题面标明依赖图片/图表时，还必须存在题图 URL 或可用的视觉事实。
...
- 只有发生答案冲突、DeepSeek 低置信度、参考答案/解析有效性矛盾、模式内容失败、缺失条件、非法选项、题面外事实，或答案虽一致但关键事实不能保守地精确覆盖时，才调用第二阶段 `deepseek_final_review`。第二阶段收到完整题目上下文、完整候选 A、完整候选 B、冲突清单和目标模式 Schema；候选来源被匿名化，不以 provider 名称影响判断。
...
- 最终集中修复后两组不重叠的相关套件共 763 项通过：纯 AI 671 项，review/DB 92 项；八项安全回归窄测 49 项和 prompt registry 24 项均已包含在前述总数中。`python manage.py check` 为 0 issue，`git diff --check` 通过。
...
- 本地实现证据：Task11 命令定向测试 15 项通过，本轮最终相关回归 763 项通过，Django `check` 为 0 issue；这些是组件和命令的模拟/本地证据，不等同于供应商调用成功。
